...
## 修改路径到数据存储的文件夹
os.chdir(r'\\file.tounawang.net\风控数据共享\工作交接_廖利明\车贷等本等息客户申请评分模型（自有模型）\数据')
...
#de_dict.to_excel('DE_Var_Select_20190808.xlsx',index=False)
...
de_dict_var = pd.read_excel('DE_Var_Select_20190808.xlsx')
...
#de_dict_vars.to_excel('de_dict_vars_20190808.xlsx',index=False)
del de_dict_vars
...
vars_count_table=pd.read_excel('de_dict_vars_20190909.xlsx') #手动增加了同盾外部评分
...
#    cat_less20 = ["deci_apt_facetrial_housetype","deci_cont02_relationship","deci_jxl_contact2_rel","deci_vehicle_minput_lastmortgagerinfo"]
    high_iv_name = v2_cat_woe_iv(df,cat_less20,target,0,"//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/") # 计算woe 和IV值
    group = []
    ...
    print(less_iv_name)
    less_iv_name.to_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/cat_less_iv_1.xlsx")
    df = v2_equif_bin(df, cat_over20, target)
    cols_name = [i+"_Bin" for i in cat_over20]
    over_iv_name = v2_cat_woe_iv(df, cols_name, target,0,"")  # 计算woe 和IV值
    over_iv_name.to_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/cat_more_iv_2.xlsx")
...
#init_choose_name_desc.to_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/init_choose_name_desc_20190909.xlsx',index=False)
init_choose_name_desc=pd.read_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/init_choose_name_desc_20190909.xlsx')
    ...
    for col in num_var:
        ...
        if -99 not in set(df[col]):
            ...
            if len(cutOff)!=0:
               ...
               regroup.to_csv("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/regroup_Chi_cutbin__.csv", mode="a", header=True)
               print(regroup)
               num_list.append(col)
...
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list,target,0,"")
df_hig_iv.to_excel("high_iv_chi.xlsx")
...
#df_choose1.to_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/cut_bin_train.xlsx")
df_hig_iv = v2_cat_woe_iv(df_choose1,cols_list+str_columns,target,0,"")
df_hig_iv.to_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/high_iv_chi.xlsx")
...
init_choose_name_desc.to_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/init_choose_name_desc_20190909v1.xlsx',index=False)
...
连续变量的相关性分析
"""
init_choose_name_desc=pd.read_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/init_choose_name_desc_20190909v1.xlsx')
init_choose_table=init_choose_name_desc.loc[init_choose_name_desc['是否选用'].isin(['是']),:]#手动去除一些不能用的变量
numeric_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff数据型','数据型', '数字型','数值型']), 'var_name'].values.tolist()
str_columns = init_choose_table.loc[init_choose_table.var_type.isin(['字符型', '\ufeff字符型','字符型 ' ]), 'var_name'].values.tolist()
date_columns = init_choose_table.loc[init_choose_table.var_type.isin(['\ufeff日期型','日期型']), 'var_name'].values.tolist()
...
error_cols,df_choose3=main(df_choose2,col,'//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/')
...
df_high_cor1.to_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/df_high_cor.xlsx")


## 相关性去除iv更低的变量
cat_less_iv=pd.read_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/cat_less_iv_1.xlsx')
cat_less_iv=cat_less_iv.set_index('var_name').loc[str_columns,'IV'].reset_index().rename(columns={'IV':'iv'})
df_hig_iv=pd.read_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/high_iv_chi.xlsx")
df_hig_iv=pd.concat([df_hig_iv,cat_less_iv],axis=0)
df_hig_iv.to_csv('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/df_hig_iv_all.csv',index=False)
df_high_cor1=pd.read_excel("//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/df_high_cor.xlsx")
...
main(df_choose5,remain_col,"//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/final_test_num_bin_badrate1")
...
main(df_choose6,remain_col,"//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/final_test_num_bin_badrate2") 
...
vif_df.to_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/vif_df_20190909.xlsx')
...
df_hig_iv = v2_cat_woe_iv(df_choose7,final_columns,'apply_y',0,"//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/")
...
main(df_choose6,final_columns,"//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/final_test_num_bin_badrate3")  
...
for cn in final_columns:
    judge_stable_analyze(temp,cn,'//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/未调整分箱前的稳定性分析/','apply_y')
...
main(df_choose6,final_columns,"//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/final_test_num_bin_badrate5")  
temp=df_choose6[final_columns+['apply_y']].copy()
temp['app_applydate']=model_data_new['app_applydate']
temp['applymonth']=temp['app_applydate'].str.slice(0,7)
for cn in final_columns:
    judge_stable_analyze(temp,cn,'//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/调整分箱后的稳定性分析/','apply_y')
...
#df_choose7.to_excel('训练集数据_20190806.xlsx')
...
lr=LogisticRegression(penalty='l2',C=2,n_jobs=-1,verbose=0,random_state=0)
...
#joblib.dump(lr,'//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/lr_alldata_20190911v1.pkl')
...
#columns_coef.to_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/columns_coef_20190911v1.xlsx',index=False)
...
de_dict_var = pd.read_excel('de_dict_vars_20190909.xlsx')
for i, _ in de_dict_var.iterrows():
    name = de_dict_var.loc[i, 'var_name']
    default = de_dict_var.loc[i, 'default']
...
#full_data.to_csv('full_data_20190722.csv',index=False)
for cn in choose_column:
    judge_stable_analyze(full_data,cn,'//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/调整分箱后的稳定性分析_加时间外验证/','apply_y')
...
print(auc_ks)
#full_data[['applymonth','contractno','app_applycode','bair_union_scores']].to_excel('百融联合建模分数_20190726.xlsx',index=False)
...
choose_cols=[cn.split('_Bin')[0] for cn in choose_column]
output_data=psi_data[['app_applycode','app_applydate']+choose_cols]
...
output_data['newloan_score3']=pred_p5*100
output_data.to_excel('//file.tounawang.net/风控数据共享/工作交接_廖利明/车贷等本等息客户申请评分模型（自有模型）/结果/2018年3月至2019年9月的所有等本等息申请样本的自有模型分数_20191016.xlsx',index=False)
